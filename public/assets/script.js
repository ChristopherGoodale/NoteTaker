...
if (window.location.pathname === '/notes') {
    ...
    newNoteBtn = document.querySelector('.write-note');
...
const saveNote = (note) =>
    fetch('/api/notes', {
        ...
        body: JSON.stringify({ note, id: Date.now().toString() }),
    });
...
const renderNoteList = async (notes) => {
    ...
    if (jsonNotes.length === 0) {
        noteListItems.push(createLi({ title: 'No saved Notes', text: '' }));
    }
...
getAndRenderNotes(); // initially fetches notes and renders them on the page.